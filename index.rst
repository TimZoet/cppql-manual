...
:code:`cppql` is a C++ library that wraps around the sqlite C API. It aims to provide a type safe interface that catches
as many potential programming errors as possible during compilation. This is done through a template based query
language that somewhat mimicks how you would write actual SQL.
...
The above example leaves out some things related to managing databases and tables. This is covered in the :doc:`core`
section. The most important part is the construction of the query. Some notes:
...
* No dynamic allocations take place until you compile the query (except for that :code:`\"abc\"` string parameter that is moved around a bit, but technically that was allocated by the user).

Of course, there are more queries you can construct for inserting, updating, deleting, etc. This is all covered in the
:doc:`/queries` section. Some additional details about writing expressions is covered in a separate :doc:`/expressions`
section.

To find out how the statements that are generated by compiling queries can be used, refer to the :doc:`/statements`
section.
